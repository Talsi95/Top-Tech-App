                        ...
                        return (
                            <div key={itemKey} className="flex items-center space-x-4 mb-4 border-b pb-2">
                                ...
                                <div className="flex-1">
                                    ...
                                    {isItemOnSale && salePrice && salePrice > 0 ? (
                                        <div className="flex items-center gap-2">
                                            <span className="text-gray-400 line-through text-xs">₪{regularPrice?.toFixed(2)}</span>
                                            <span className="text-red-500 font-semibold text-sm">₪{effectivePrice?.toFixed(2)}</span>
                                            <span className="text-gray-600 text-xs"> x {item.quantity}</span>
                                        </div>
                                    ) : (
                                        <p className="text-gray-600 text-xs">₪{regularPrice?.toFixed(2)} x {item.quantity}</p>
                                    )}

                                    <p className="text-gray-800 font-bold mt-1">סה"כ: ₪{itemTotal.toFixed(2)}</p>
                                </div>
