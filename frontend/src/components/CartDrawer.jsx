            ...
            <div className="p-4 overflow-y-auto" style={{ maxHeight: 'calc(100% - 150px)' }}>
                {cartItems.length === 0 ? (
                    ...
                    cartItems.map(item => {
                        ...
                        if (!item.product) return null; // Prevents rendering deleted products

                        return (
                            <div key={itemKey} className="flex items-center space-x-4 mb-4 border-b pb-2">
                                <img src={itemImageUrl} alt={item.product?.name} className="w-16 h-16 object-cover rounded" />
                                <div className="flex-1">
                                    <h3 className="font-semibold text-sm">{item.product.name}</h3>
                                    {item.variant && (
                                        <p className="text-gray-600 text-xs">{item.variant.color} | {item.variant.storage}</p>
                                    )}
                                    <p className="text-gray-600 text-xs">₪{itemPrice?.toFixed(2)} x {item.quantity}</p>
                                    <p className="text-gray-800 font-bold">₪{itemTotal.toFixed(2)}</p>
                                </div>
...
//                 ) : (
//                     cartItems.map(item => (
//                         <div key={item.product._id + item.variant._id} className="flex items-center space-x-4 mb-4 border-b pb-2">
//                             <img src={item.variant.imageUrl} alt={item.product.name} className="w-16 h-16 object-cover rounded" />
//                             <div className="flex-1">
//                                 <h3 className="font-semibold text-sm">{item.product.name}</h3>
//                                 <p className="text-gray-600 text-xs">{item.variant.color} | {item.variant.storage}</p>
//                                 <p className="text-gray-600 text-xs">₪{item.variant.price.toFixed(2)} x {item.quantity}</p>
//                                 <p className="text-gray-800 font-bold">₪{(item.variant.price * item.quantity).toFixed(2)}</p>
//                             </div>
//                             <div className="flex items-center">
//                                 <button
//                                     onClick={() => onUpdateQuantity(item.product._id, item.variant._id, 'decrease')}
